cv_clas_hyper <- function(dataset,k,classifier,seed=123, thres= 0.5) {
  ...
  for (i in 1:k) {
    ...
    if(classifier == "xgb"){
      ...
      train_x <- model.matrix(Night_class_hyper_30 ~ .,
                              data = data_train,)[,-15]
      train_y <- as.numeric(data_train$Night_class_hyper_30) 
      ...
      conf <- table(true = data_valid$Night_class_hyper_30,predicted = xgb_test)
    }
    
    if(classifier == "lr"){
    model <- glm(Night_class_hyper_30 ~ ., family=binomial, data= data_train)
    pred_prob <- predict(model, type = "response", newdata = data_valid)
    pred_lr <- ifelse(pred_prob < as.numeric(thres), 0,1)
    
    conf <- table(true = data_valid$Night_class_hyper_30, predicted = pred_lr)
    }
